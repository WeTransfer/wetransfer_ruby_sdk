...
describe WeTransfer::Client do
  ...
  describe "initializer" do
    subject(:client) { described_class.new(api_key: 'fake key')}
    ...
    it "needs an :api_key kw param" do
      ...
      expect { described_class.new(api_key: 'fake key') }
        .to_not raise_error(ArgumentError, %r|api_key|)
    end

    it "resets credentials (to prevent caching outdated credentials)" do
      expect(WeTransfer::Communication)
        .to receive(:reset_authentication!)

      subject
    end

    it "sends the api_key to the Comm" do
      expect(WeTransfer::Communication)
        .to receive(:reset_authentication!)
